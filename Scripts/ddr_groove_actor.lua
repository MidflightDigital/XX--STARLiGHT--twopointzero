...
local function RadarItems(pn, size, center_color, category_colors, tween_type, tween_time)
	...
	for j=1, 5 do
		t[#t+1] = Def.Quad {
			Name="r" .. j;
			--Condition=ThemePrefs.Get("WheelType") ~= "CoverFlow" and ThemePrefs.Get("WheelType") ~= "Wheel";
			InitCommand=cmd(valign,1;setsize,2,size;rotationz,-72*(j-1);zoomy,0;diffuse,color('0,0,0,1'));
			["CurrentSongChangedMessageCommand"]=function(self)
				self:playcommand('Set')
			end;
			["CurrentSteps" .. pname(pn) .. "ChangedMessageCommand"]=function(self)
	...
	t[#t+1] = Def.BitmapText { Font="Common Normal" } .. {
		Name="failed_lookup",
		Text= "Not Found",
		InitCommand=cmd(visible,false;diffuse,color('0.8,0.8,0.8,0.8')),
	}
