...
local inSearchMode = false
local enteredSearch = false
...
local function UpdateSortingFolders(self)
...
return function(Style)
    ...
    return Def.ActorFrame{
		...
		StartCommand=function(self)
			...
			if Joined[self.pn] then 
				...
				if type(GroupsAndSongs[CurSong]) == "string" then
					...
					GroupsAndSongs = LoadModule("Wheel/Group.Sort.lua")(Songs,"")
					MoveSelection(self,0,GroupsAndSongs)
					...
					GroupsAndSongs = LoadModule("Wheel/Group.Sort.lua")(Songs,CurGroup)
					MoveSelection(self,0,GroupsAndSongs)
		...
		CodeMessageCommand=function(self,param)
			if param.Name=="NextDiff" then
				MoveDifficulty(self,1,GroupsAndSongs)
			end
			if param.Name=="PrevDiff" then
				MoveDifficulty(self,-1,GroupsAndSongs)
			end
